...
chat_session = model.start_chat(history=[
    ...
    },
    {
      "role": "model",
      "parts": [
        "No worries at all!  Have a fantastic day, and don't  conform hesitate to reach out if you need anything else. 😊 \n",
      ],
    },
  ])
...
def generate_message(message_body, phone_number):
    ...
    conversation_states = ["enquiry", "booking", "complaints and feedback", "confirm", "confirm_yes_no"]

    j = chat_session.history[-1].role
    y = chat_session.history[-1].parts
    y = str(y)
    desired_string = y.strip('[text: "').strip('" ]')

    if j == "model" and "confirm" in desired_string and current_state is "booking":
        current_state = conversation_states[3]
        

    # Extract conversation state from message_body
    message_body_lower = message_body.lower()
    if "enquiry" in message_body_lower:
        current_state = conversation_states[0]
    elif "booking" in message_body_lower:
        current_state = conversation_states[1]
    elif "complaints and feedback" in message_body_lower:
        current_state = conversation_states[2]
    ...
    if current_state == "enquiry":
        ...
        today  = date.today()
        datei=datetime.strptime(values[3], "%d/%m/%Y").date()
        
        
        print(text)
        print(values)
        del chat_session.history[-1]
        del chat_session.history[-1]
        if datei < today:
            current_state = conversation_states[1]
            chat_session.send_message("user enters past date takes correct date from user after taking correct date ask user to type confirm ")
            return "you enter past date please enter corrrect date"
        ...
        if api_response_drop and api_response_pick and ty['availability']:
            # Tell agent to confirm booking
            #agent_response = tell_agent_to_confirm_booking()
            current_state = conversation_states[4]  # Move to confirm_yes_no state
            
            return f"We are servicing here and slot is also available, the fare is {Fare}.{send_template_message_yes(phone_number)}" 
        else:

            current_state = conversation_states[1]
            chat_session.send_message("confirm")

            return "we are not servicing here or slot is not avaible"
        
    elif current_state == "confirm_yes_no":
        if message_body_lower == "yes":
            # Confirm booking
            # Add code to confirm booking here
            current_state = conversation_states[0]
            chat_session.send_message("confirm")
            return "Congratulations! Your eco-friendly ride with GoBlu-EV has been confirmed. Driver details will be shared 30 mins before the ride starts.Check our app for the ride details. For any assistance call +97148718604 or mail support@noorride.com.Have a safe ride."
        elif message_body_lower == "no":
            # Cancel booking
            # Add code to cancel booking here
            current_state = conversation_states[0]
            chat_session.send_message("booking cancelled")
            return "Booking cancelled!"
        else:
            current_state = conversation_states[4]

            return f"Invalid response. Please respond with 'yes' or 'no'.{send_template_message_yes(phone_number)}"
...
def process_whatsapp_message(body):
    ...
    if(message_type == "text"):
        ...
        response = generate_message(message_body,message_from)
